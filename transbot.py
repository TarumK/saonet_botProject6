...
def start(m, res=False):
    bot.send_message(m.chat.id, 'Русско-черкесский бот-переводчик.\n'
                                'ver 0.1(beta). Источник словарной базы: academic.ru\n\n'
                                'Наберите слово на русском языке...')
# Получение сообщений от юзера
@bot.message_handler(content_types=["text"])
def handle_text(message):
    bot.send_message(message.chat.id, (scrap.translate(message.text)).isupper())
